...
PRIMITIVES['sleep'].isBlocking = true;
PRIMITIVES['sleep'].cases[0].isBlocking = true;
PRIMITIVES['sleep'].cases[1].isBlocking = true;
...
PRIMITIVES['js-big-bang'].isBlocking = true;
...
PRIMITIVES['make-world-config'] =
    makeOptionPrimitive('make-world-config',
			...
			[false, false],
			false,
			function(userArgs, startup, shutdown, pauser, restarter) {
				...
				check(pauser, function(x) { return (x === false || procArityContains(1)(x)); },
				      'make-world-config', 'procedure (arity 1) or #f', 3, userArgs);
				check(restarter, function(x) { return (x === false || procArityContains(2)(x)); },
				      'make-world-config', 'procedure (arity 2) or #f', 4, userArgs);

				return types.worldConfig(startup, shutdown, pauser, restarter);
			});
...
PRIMITIVES['bb-info-call-with-pausing'] =
    new PrimProc('bb-info-call-with-pausing',
		 1,
		 false, false,
		 function(bbInfo) {
		 	check(bbInfo, types.isBigBangInfo, 'bb-info-call-with-pausing', 'bb-info', 1);
			return types.bbInfoCallWithPausing(bbInfo);
		 });

PRIMITIVES['bb-info-unsafe-change-world'] =
    new PrimProc('bb-info-unsafe-change-world',
		 1,
		 false, false, 
		 function(bbInfo) {
		 	check(bbInfo, types.isBigBangInfo, 'bb-info-unsafe-change-world', 'bb-info', 1);
			return types.bbInfoUnsafeChangeWorld(bbInfo);
		 });
...
PRIMITIVES['compound-effect?'] = new PrimProc('compound-effect?', 1, false, false, isCompoundEffect);
