...
//    var beforeCallListeners = aState.beforeCallListeners.slice(0);
//    var afterCallListeners = aState.afterCallListeners.slice(0);
...
//		helpers.forEachK(afterCallListeners,
//				 function(aListener, k2) { aListener(k2); },
//				 aState.onFail,
//				 function() {
					aState.restore(stateValues);
					k(v);
...
//    };
//
//    helpers.forEachK(beforeCallListeners,
//		     function(aListener, k2) { aListener(k2); },
//		     aState.onFail,
//		     runCall);
};
...
var callCC = new primitive.Primitive('call/cc',
				     1,
				     false, true,
				     function(state, f) {
					var continuationClosure = captureContinuationClosure(state);
					state.pushValue(continuationClosure);
					state.v = f;
					state.pushControl(new control.CallControl(1));
				     });
callCC.isBlocking = true;
primitive.addPrimitive('call/cc', callCC);
