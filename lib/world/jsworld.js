...
(function() {
	...
	var BigBang = function(aState, caller) {
		...
		this.beforeCallPause = function(k) { that.pause(k); };
		this.afterCallRestart = function(k) { that.restart(k); };
		...
		this.initConfigStartupArg();
	};

	BigBang.prototype.initConfigStartupArg = function() {
		var that = this;
		var schemeChangeWorld =
			new types.PrimProc('change-world', 1, false, false,
				function(updater) {
					...
					if ( that.hasEnded || that.isPaused ) {
						helpers.raise(types.incompleteExn(
								types.exnFail,
								'change-world: cannot change the world while the big bang is not running',
								[]));
					}
					return PAUSE(function(newCaller, newOnSuccess, newOnFail) {
						that.changeWorld(function(w, k) {
							that.pause(function () { newCaller(updater,
											   [w],
											   function (res) {
											   	that.restart(function() { k(res); })
											   },
											   function(e) { throw e; },
											   'change-world'); })
							},
		...
		var schemeCallWithPausing =
			new types.PrimProc('call-with-pausing', 1, false, false,
				function(thunk) {
					helpers.check(thunk, helpers.procArityContains(0),
						      'call-with-pausing', 'procedure (arity 0)', 1);
					// Make sure the world is currently running
					if (that.hasEnded) {
						helpers.raise(types.incompleteExn(
								types.exnFail,
								'call-with-pausing: cannot pause a big bang that has ended.',
								[]));
					}

					if ( that.paused ) {
						return types.internalCall(thunk, [], function(x) { return x; });
					}
					else {
						return PAUSE(function(newCaller, newOnSuccess, newOnFail) {
							that.pause(function() {
								newCaller(thunk,
									  [],
									  function(res) {
										that.restart(function() { newOnSuccess(res); });
									  },
									  newOnFail,
									  'call-with-pausing');
							});
		...
		var schemeUnsafeChangeWorld =
			new types.PrimProc('unsafe-change-world', 1, false, false,
				function(updater) {
					helpers.check(updater, helpers.procArityContains(1),
						      'unsafe-change-world', 'procedure (arity 1)', 1);
					if ( that.hasEnded || that.isPaused ) {
						helpers.raise(types.incompleteExn(
								types.exnFail,
								'unsafe-change-world: cannot change the world while the big bang is not running',
								[]));
					}
					return PAUSE(function(newCaller, newOnSuccess, newOnFail) {
		...
		this.configStartupArg = types.makeBigBangInfo(helpers.wrapJsValue(this.toplevelNode),
							      schemeChangeWorld,
							      schemeCallWithPausing,
							      schemeUnsafeChangeWorld);
	};
	...
	BigBang.prototype.setOnSuccess = function(k) {
		...
		this.onSuccess = function(v) {
			...
			that.hasEnded = true;
			that.state.noBlockingCalls = false;
			k(v);
		};
	};
...
//	BigBang.prototype.setCallListeners = function() {
//		this.state.addBeforeCallListener(this.beforeCallPause);
//		this.state.addAfterCallListener(this.afterCallRestart);
//	};
//
//	BigBang.prototype.unsetCallListeners = function() {
//		this.state.removeBeforeCallListener(this.beforeCallPause);
//		this.state.removeAfterCallListener(this.afterCallRestart);
//	};

	BigBang.prototype.start = function(initWorld) {
		...
		this.running = true;
		this.paused = false;
		this.state.noBlockingCalls = true;
		this.changeWorld(function(w, k) { k(initWorld); },
				 function() { that.startUserConfigs(doNothing); });
	};

	BigBang.prototype.pause = function(k) {
		if ( !this.running || this.paused ) {
			k();
			return;
		}
		var that = this;
		this.paused = true;
//		this.unsetCallListeners();
		this.pauseUserConfigs(function() {
					that.state.noBlockingCalls = false;
					k();
				      });
	};

	BigBang.prototype.restart = function(k) {
		if ( !this.running || !this.paused ) {
			k();
			return;
		}
		...
		this.paused = false;
		this.state.noBlockingCalls = true;
		this.restartUserConfigs(k);
//					function() {
//						that.setCallListeners();
//						k();
//					});
	};
...
//		// If we're not currently running, then raise an exception because this is illegal
//		// This is a failsafe against people accidentally changing the world
//		// when they shouldn't be
//		if ( this.hasEnded || this.paused ) {
//			this.state.onFail(types.schemeError(types.incompleteExn(
//					types.exnFail,
//					'change-world: cannot change the world while a big-bang is paused or after it has finished',
//					[])));
////			k();
//			return;
//		}
	...
	BigBang.prototype.startUserConfigs = function(k) {
		...
		helpers.forEachK(this.userConfigs,
				 function(aConfig, k2) {
				 	// Only start the config if it's not already running
				 	if ( !aConfig.running ) {
						that.caller(aConfig.startup,
							    [that.configStartupArg],
							    function(res) {
							    	aConfig.running = true;
								aConfig.paused = false;
	...
	BigBang.prototype.pauseUserConfigs = function(k) {
//		console.log('pausing!');
		var that = this;
		helpers.forEachK(this.userConfigs,
				 function(aConfig, k2) {
				 	// If the config is not running or it's paused, leave it alone
				 	if ( !aConfig.running || aConfig.paused ) {
						k2();
					}
					// Otherwise, if it has a pauser use it
					else if ( aConfig.pauser ) {
						that.caller(aConfig.pauser,
							    [aConfig.shutdownArg],
							    function(res) {
							    	aConfig.paused = true;
					...
					// If not, use the shutdown procedure instead
					else {
						that.caller(aConfig.shutdown,
							    [aConfig.shutdownArg],
							    function(res) {
							    	aConfig.running = false;
							    	aConfig.paused = true;
							    	aConfig.restartArg = res;
	...
	BigBang.prototype.restartUserConfigs = function(k) {
//		console.log('restarting!');
		var that = this;
		helpers.forEachK(this.userConfigs,
				 function(aConfig, k2) {
				 	// Only restart the config if it's not running or it's paused
				 	if ( aConfig.paused ) {
						// If there's not restarter, default to
						// using the startup procedure
						var restarter = aConfig.startup;
						var args = [that.configStartupArg];
						if ( aConfig.restarter ) {
							restarter = aConfig.restarter;
							args.push(aConfig.restartArg);
						}
						that.caller(restarter,
							    args,
							    function(res) {
							    	aConfig.running = true;
								aConfig.paused = false;
	...
	BigBang.prototype.shutdownUserConfigs = function(k) {
		...
		helpers.forEachK(this.userConfigs,
				 function(aConfig, k2) {
				 	// If the config isn't running, then there's nothing to
				 	// shutdown, so just move on
				 	if ( aConfig.running ) {
						that.caller(aConfig.shutdown,
							    [aConfig.shutdownArg],
							    function(res) {
...
//		}
	...
	var doRedraw = function(aBigBang, w, redrawFun, redrawCssFun, k) {
		...
		maintainingSelection(
		    function(k2) {
			aBigBang.pause(function() {
			    redrawFun(w, function(newPage) {
				redrawCssFun(w, function(newCss) {
					redrawPage(newPage, newCss, function() { aBigBang.restart(k2); });
				});
			    });
			});
		    }, k);
