...
Vector.prototype.isEqual = function(other, aUnionFind) {
	if ( !(other instanceof Vector) || other.length() != this.length() ) {
		return false;
	}
...
var WorldConfig = function(startup, shutdown, pauser, restarter) {
...
var BigBangInfo = makeStructureType('bb-info', false, 4, 0, false,
			function(args, name, k) {
				helpers.check(args[0], types.isJsValue, name, 'js-object', 2);
				helpers.check(args[1], helpers.procArityContains(1), name, 'procedure (arity 1)', 1);
				helpers.check(args[2], helpers.procArityContains(1), name, 'procedure (arity 1)', 3);
				helpers.check(args[3], helpers.procArityContains(1), name, 'procedure (arity 1)', 1);
				return k(args);
			});
types.BigBangInfo = BigBangInfo;
types.makeBigBangInfo = BigBangInfo.constructor;
types.isBigBangInfo = BigBangInfo.predicate;
types.bbInfoToplevelNode = function(info) { return BigBangInfo.accessor(info, 0); };
types.bbInfoChangeWorld = function(info) { return BigBangInfo.accessor(info, 1); };
types.bbInfoCallWithPausing = function(info) { return BigBangInfo.accessor(info, 2); };
types.bbInfoUnsafeChangeWorld = function(info) { return BigBangInfo.accessor(info, 3); };
