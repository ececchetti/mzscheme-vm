...
var callPrimitiveProcedure = function(aState, procValue, n, operandValues) {
    if ( aState.noBlockingCalls && procValue.isBlocking ) {
	    helpers.raise(types.exnFail('Attempted to make blocking call to ' + procValue.name +
				    	   ' when blocking calls are not allowed',
				        state.captureCurrentContinuationMarks(aState)));
    }
    ...
    var args = preparePrimitiveArguments(aState, 
					 procValue, 
					 operandValues,
					 n);
...
var processPrimitiveResult = function(aState, result, procValue) {
    if ( types.isInternalCall(result) ) {
	aState.cstack.push(new InternalCallRestartControl
			  (result.k, procValue));
	callProcedure(aState,
		      result.operator, 
